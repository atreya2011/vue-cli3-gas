<template>
  <v-card>
    ...
    <v-card-text>
      <v-layout column>
        <v-flex xs12>
          <v-text-field label="対象ファイル" v-model="sourceFile.name" @click="pickSourceFile" solo append-icon="insert_drive_file" readonly hide-details />
        </v-flex>
        <v-flex xs12>
          <v-text-field label="作成先フォルダ" v-model="targetFolder.name" @click="pickTargetFolder" solo append-icon="folder" readonly hide-details />
        </v-flex>
        <v-flex xs12>
    ...
    <v-card-actions>
      <v-btn v-if="copiedFile.id" flat color="primary" :href="copiedFile.url" target="_blank">
        ファイルを開く
        <v-icon right>open_in_new</v-icon>
      </v-btn>
      <v-btn v-else color="primary" @click="execute" :disabled="!ready || executing" :loading="executing">
        新規作成
      </v-btn>
    </v-card-actions>
